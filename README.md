...
STM32F767ZI（Nucleo-144系ボード）をベースにした、オムニ駆動の走行ロボット制御ファームウェア。プロポ（送信機）からのSBUS入力、CANバス経由の外部コマンド、2基のLidar距離センサーを組み合わせて、走行とローラー機構（吸引/収集用）を制御する。

## 概要

- **駆動方式**: 4輪オムニ駆動（前左/前右/後左/後右）
...
| `-1` | 全自動モード | 2つのLidar距離から前後（`auto_ly`）・回頭（`auto_rx`）を自動算出し、目標距離、壁に対して平行かつ一定距離を保持 |
